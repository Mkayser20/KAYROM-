...
<!-- Alerta de stock bajo: franja compacta arriba de todo, no un panel entero -->
<?php if (!empty($data['lowStockItems'])): ?>
<div class="alert-strip">
    <div class="alert-strip-icon">
    ...
    <div class="alert-strip-text">
        <b><?= count($data['lowStockItems']) ?></b> repuesto(s) con stock bajo:
        <?php foreach ($data['lowStockItems'] as $i => $item): ?>
            <?= $i > 0 ? '<span class="item-sep"> · </span>' : '' ?><?= htmlspecialchars($item['nombre']) ?> (<?= $item['stock'] ?>)
        <?php endforeach; ?>
    </div>
    <a href="index.php?page=repuestos" class="btn btn-ghost" style="font-size:11px;padding:6px 12px;flex-shrink:0;">Ver Detalles &rsaquo;</a>
</div>
<?php else: ?>
<div class="alert-strip" style="border-left-color:var(--accent-green);">
    <div class="alert-strip-icon" style="color:var(--accent-green);">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
    </div>
    <div class="alert-strip-text">Todo el stock está OK, sin repuestos por debajo del mínimo.</div>
</div>
<?php endif; ?>
